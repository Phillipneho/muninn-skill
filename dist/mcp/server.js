...
import { routeContent } from '../extractors/index.js';
...
async function handleToolCall(name, args) {
    switch (name) {
        case 'memory_remember': {
            ...
            const hasResolved = result.resolved_content !== null;
            return {
                success: true,
                id: result.id,
                type: result.type,
        ...
        case 'memory_briefing': {
            ...
            const priming = await cohesionQuery(store.getEntityStore(), store.getRelationshipStore(), async (q, opts) => store.recall(q, opts), { minMentions: 3, staleHours: 48, maxEntities: 5, memoriesPerEntity: 2 });
            // Generate summary
