...
export class TemporalDecayScorer {
     ...
     */
    score(memory, referenceDate = new Date()) {
     ...
     */
    applyDecay(memories, scoreProperty = '_finalScore', referenceDate = new Date()) {
        return memories.map(mem => {
            const temporalScore = this.score(mem, referenceDate);
            const baseScore = mem[scoreProperty] || 1.0;
     ...
     */
    sortByTemporalScore(memories, scoreProperty = '_finalScore') {
 ...
 */
export function getContradictionChain(entityStore, relationshipStore, entityName) {
 ...
 */
export function applyTemporalDecayWithContradictions(memories, baseScores, entityStore, relationshipStore, options = {}) {
    const scorer = new TemporalDecayScorer(options);
    const referenceDate = new Date();
    // First, collect all entities that have contradictions
    const contradictionEntities = new Map();
 ...
 */
export function getTemporalScore(memory, halfLifeDays = 30) {
