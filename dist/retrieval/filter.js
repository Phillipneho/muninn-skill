 ...
 */
export async function filterMemories(query, memories, options = {}) {
    ...
    try {
        const result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('LLM timeout')), timeout))
        ]);
 ...
 */
export function scoreForQuestionType(memories, query) {
 ...
 */
async function llmFilter(query, memories, options) {
...
function parseLLMResponse(response) {
    try {
        ...
        for (const key of Object.keys(parsed)) {
            if (Array.isArray(parsed[key]) && parsed[key].every((v) => typeof v === 'number')) {
 ...
 */
async function generateLLMResponse(prompt, timeoutMs = 10000) {
    // Use global mock if available (for testing)
    if (typeof globalThis.generateLLMResponse === 'function') {
        return globalThis.generateLLMResponse(prompt);
    }
