 ...
 */
function filterByTemporal(memories, constraint) {
    ...
    return memories.filter(m => {
        ...
        if (!memDate && m.created_at) {
            memDate = new Date(m.created_at);
        }
        if (!memDate)
            return false;
        if (constraint.on) {
 ...
 */
export async function retrieveEntityCentric(query, getMemoriesByEntity, options = {}) {
    ...
    const scored = filtered.map(m => {
        ...
        const entityCount = (m.entities || []).filter(e => entities.some(qe => e.toLowerCase().includes(qe.toLowerCase()))).length;
 ...
 */
function computeConfidence(memories, query, entities, temporalConstraint) {
    if (memories.length === 0)
        return 0;
    let score = 0;
    // 1. Entity coverage (40%)
    const memoryEntities = new Set(memories.flatMap(m => (m.entities || []).map(e => e.toLowerCase())));
    ...
    if (temporalConstraint.after || temporalConstraint.before) {
        const satisfyingMemories = memories.filter(m => {
            ...
            if (!memDate)
                return false;
            if (temporalConstraint.after && memDate <= temporalConstraint.after)
                return false;
            if (temporalConstraint.before && memDate >= temporalConstraint.before)
                return false;
            return true;
        });
    ...
    const queryWords = query.toLowerCase()
        ...
        .filter(w => w.length > 3);
    const keywords = expandKeywords(queryWords);
    const keywordMatches = keywords.filter(kw => memories.some(m => m.content.toLowerCase().includes(kw)));
 ...
 */
export async function multiHopRetrieval(query, getMemoriesByEntity, options = {}) {
    ...
    if (options.entityStore && options.relationshipStore && options.allMemories) {
        try {
            expandedMemories = await spreadActivation(entityMemories, options.relationshipStore, options.entityStore, options.allMemories, {
...
//# sourceMappingURL=multi-hop.js.map