...
export class MemoryStore {
    ...
    async remember(content, type = 'semantic', options = {}) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, type, content, options.title || null, options.summary || null, JSON.stringify(canonicalEntities), JSON.stringify(options.topics || []), embeddingBuffer, options.salience || 0.5, timestamp, options.sessionId || null, options.ttl || null, now, now);
