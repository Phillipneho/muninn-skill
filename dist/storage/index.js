...
import { scoreForQuestionType } from '../retrieval/filter.js';
...
export class MemoryStore {
    ...
    `);
        ...
        for (const rel of extractedRels) {
            ...
            if (!sourceEntity) {
                sourceEntity = this.entityStore.addEntity({
                    name: rel.source,
                    type: inferEntityType(rel.source),
                    aliases: []
                });
            }
            ...
            if (!targetEntity && !rel.value) {
                targetEntity = this.entityStore.addEntity({
                    name: rel.target,
                    type: inferEntityType(rel.target),
                    aliases: []
                });
            }
      ...
      `);
            for (const td of extractedTemporalDates) {
                ...
                temporalStmt.run(tdId, id, dateStr, td.dateType, td.granularity, td.originalText, td.confidence);
                ...
                if (td.endDate) {
                    ...
                    temporalStmt.run(tdIdEnd, id, endDateStr, td.dateType, td.granularity, td.originalText + ' (end)', td.confidence);
                }
            }
        }
      ...
      `).all(targetDate);
        ...
        if (temporalQueryDetection.isTemporal || queryDates.length > 0) {
          ...
          `).all(dateStr);
            ...
            `).all(rangeStart.toISOString().split('T')[0], rangeEnd.toISOString().split('T')[0]);
     ...
     */
    async recallInternal(context, memories, limit) {
        try {
            ...
            if (queryEntities.length > 0) {
                for (const c of candidates) {
                    ...
                    const overlap = memoryEntities.filter((e) => queryEntities.some((qe) => e.toLowerCase() === qe.toLowerCase()));
                    if (overlap.length > 0) {
            ...
            candidates.sort((a, b) => {
                const scoreA = a._finalScore || a._rrfScore || 0;
                const scoreB = b._finalScore || b._rrfScore || 0;
                const boostA = a._entityBoost || 1;
                const boostB = b._entityBoost || 1;
    ...
    async simpleRecall(context, limit, user_id) {
      ...
      `).all();
        ...
        return scored.slice(0, limit).map(({ _similarity, ...m }) => m);
    }
    ...
    // Graph edges
    connect(sourceId, targetId, relationship) {
    ...
    getNeighbors(memoryId, depth = 1) {
        const neighbors = new Set([memoryId]);
        const result = [];
    ...
    // Stats
    getStats() {
        const total = this.db.prepare('SELECT COUNT(*) as count FROM memories WHERE deleted_at IS NULL').get().count;
        const byType = {
        ...
        const procedures = this.db.prepare('SELECT COUNT(*) as count FROM procedures').get().count;
        return { total, byType, entities, edges, procedures };
    }
    // Procedure management
    async createProcedure(title, steps, description) {
        const id = `proc_${uuidv4().slice(0, 8)}`;
        const now = new Date().toISOString();
        const procedureSteps = steps.map((desc, i) => ({
    ...
    getAllProcedures() {
        const rows = this.db.prepare('SELECT * FROM procedures ORDER BY updated_at DESC').all();
        return rows.map(row => ({
            ...row,
            steps: JSON.parse(row.steps || '[]'),
            evolution_log: JSON.parse(row.evolution_log || '[]'),
    ...
    async procedureFeedback(procedureId, success, failedAtStep, context) {
     ...
     */
    getMemoriesByEntity(entityName, limit = 50) {
    ...
    `).all();
        ...
        const matching = memories.filter(m => {
            ...
            if (entities.some((e) => patterns.includes(e.toLowerCase()))) {
                return true;
            }
     ...
     */
    async recallEntityContext(query, options = {}) {
        const limit = options.limit || 20;
