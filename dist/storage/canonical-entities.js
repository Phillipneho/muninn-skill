...
export class CanonicalEntityTable {
     ...
     */
    register(name, type, aliases = [], metadata) {
        // Check if exists
    ...
    `).get(type);
      ...
      VALUES (?, ?, ?, ?, ?, ?, 1, ?)
    `).run(id, name, JSON.stringify([name, ...aliases]), type, new Date().toISOString(), new Date().toISOString(), metadata ? JSON.stringify(metadata) : null);
        return {
            id,
            canonicalName: name,
...
//# sourceMappingURL=canonical-entities.js.map