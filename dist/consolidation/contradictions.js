 ...
 */
export async function detectContradiction(mem1, mem2) {
    // Skip if same memory
    if (mem1.id === mem2.id)
        return null;
...
function checkNumericContradiction(content1, content2) {
    ...
    const hasSharedContext = contextWords.some(w => content1.toLowerCase().includes(w) && content2.toLowerCase().includes(w));
    if (!hasSharedContext)
        return null;
...
function checkPriorityContradiction(content1, content2) {
    ...
    const priorities1 = priorityWords.filter(w => new RegExp(`\\b${w}\\b`, 'i').test(content1));
    const priorities2 = priorityWords.filter(w => new RegExp(`\\b${w}\\b`, 'i').test(content2));
 ...
 */
export async function scanContradictions(store) {
 ...
 */
export async function getResolvedValue(store, query) {
    const memories = await store.recall(query, { limit: 5 });
    if (memories.length === 0) {
        return { value: '', source: null, confidence: 0 };
    }
    ...
    if (contradictions.length === 0) {
        ...
        const mostRecent = memories.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
        return {
