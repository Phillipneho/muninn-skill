 ...
 */
export async function consolidate(store, options = {}) {
    ...
    for (let i = 0; i < allMemories.length; i++) {
        for (let j = i + 1; j < allMemories.length; j++) {
            const shared = allMemories[i].entities.filter(e => allMemories[j].entities.includes(e));
...
export function resolveTruth(claims) {
    // Sort by timestamp (newest first)
    const sorted = [...claims].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    const resolved = [];
    const seen = new Map();
