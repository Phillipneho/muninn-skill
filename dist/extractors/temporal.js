...
export function extractDates(text, referenceDate) {
    ...
    if (standaloneDayMatch && !text.match(/\b(next|last)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i)) {
        ...
        let diff = ref.weekday - 1 - targetDay;
        if (diff < 0)
            diff += 7; // Target is earlier this week, go back
        const dt = ref.minus({ days: diff });
        addResult({
            date: dt.toJSDate(),
            dateType: 'relative',
 ...
 */
function parseExplicitDate(dateStr, ref) {
 ...
 */
export function formatDateRange(start, end, format = 'readable') {
