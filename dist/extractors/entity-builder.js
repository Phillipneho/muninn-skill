 ...
 */
export function registerEntity(name, type, timestamp) {
    const c = loadCache();
 ...
 */
export function findEntity(nameOrAlias) {
    const c = loadCache();
 ...
 */
export function mergeEntities(sourceId, targetId) {
    const c = loadCache();
 ...
 */
export async function extractEntitiesFromText(text, timestamp) {
...
Entities (JSON array):`;
    try {
        ...
        for (const e of entities) {
            if (e.name && e.type) {
                const registered_entity = registerEntity(e.name, e.type, timestamp);
 ...
 */
export async function scanMemoriesForEntities(memories) {
    let newEntities = 0;
