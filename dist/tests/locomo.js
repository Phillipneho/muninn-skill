 ...
 */
async function synthesizeAnswer(query, memories) {
    ...
    if (mainEntity && (isTemporalQuery || isContradictionQuery)) {
        relevantMemories = memories.filter(m => m.content.toLowerCase().includes(mainEntity.toLowerCase()) ||
...
function scoreAnswer(q, answer, retrieved) {
    ...
    const hasRelevantSession = retrieved.some(m => q.requiresSessions.some(s => (m.topics && m.topics.includes(s)) ||
        (m.sessionId === s)));
...
//# sourceMappingURL=locomo.js.map