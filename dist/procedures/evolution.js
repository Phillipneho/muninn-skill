 ...
 */
export async function analyzeFailure(procedure, failedAtStep, context) {
  ...
  "confidence": <0.0-1.0>
    ...
    try {
        ...
        return {
            ...
            newSteps: procedure.steps.map((s, i) => i + 1 === failedAtStep
 ...
 */
export function calculateReliability(procedure) {
 ...
 */
export function shouldAutoEvolve(procedure) {
 ...
 */
export async function evolveProcedure(procedure, failedAtStep, context) {
    const analysis = await analyzeFailure(procedure, failedAtStep, context);
    // Create new steps from analysis
    const newSteps = analysis.newSteps.map((desc, i) => ({
